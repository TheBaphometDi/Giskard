...
def load_api_keys():
    """Загружает API ключи из файла Key.json"""
...
def get_excerpt_by_gemini(gemini_model):
    """Получает отрывок из романа 'Мастер и Маргарита'"""
...
def generate_questions_with_gemini(gemini_model, excerpt):
    """Генерирует вопросы на основе отрывка"""
...
Важно: верни только валидный JSON без дополнительного текста."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Исправление распространённых ошибок JSON
            ...
            for qa in questions[:20]:
                if isinstance(qa, dict) and 'question' in qa and 'answer' in qa:
                    ...
                    # Проверка на минимальную длину и валидность
...
def get_answers_from_gemini(gemini_model, questions, excerpt):
    """Получает ответы от Gemini на сгенерированные вопросы"""
    answers = []
        ...
        for attempt in range(max_retries):
            try:
                ...
                if answer and len(answer) > 10:
                    # Дополнительная проверка на качество ответа
...
def prepare_test_data():
    """Основная функция подготовки данных для тестирования"""
    print("=" * 50)
